...
const SavedFilter = () => {
  const { lightMode, handleRemote, isRemoteChecked, setIsRemoteChecked, search, handleSearch, selectedCity, handleSelect, cities} = useContext(UserContext);
...
<IoLocation size={21}  className={lightMode? "icon-light": "icon"}  />
  ...
  <p className="remote">Remote Only</p>
   ...
   <Link to="/"><button className={lightMode? "savedjobs-light": "savedjobs"} >  home page</button></Link>
